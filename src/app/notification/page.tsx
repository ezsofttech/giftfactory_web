...
export default function NotificationPage() {
  ...
  // Auth Guard Effect
  useEffect(() => {
    if (status === 'unauthenticated' && !authOpenedRef.current) {
  ...
  if (status === 'loading' || (status === 'authenticated' && isLoading)) {
    return (
      <div className="container mx-auto px-4 py-16 min-h-screen flex flex-col items-center justify-center bg-slate-50/30">
        <div className="w-8 h-8 border-2 border-slate-300 border-t-pink-500 rounded-full animate-spin mb-3" />
  ...
  return (
    <div className="container mx-auto px-4 py-8 min-h-screen flex flex-col items-center justify-start bg-slate-50/30">
      <div className="w-full max-w-2xl flex flex-col items-center mb-6">
        <h1 className="text-2xl font-bold text-slate-800 mb-2">Notification Center</h1>
        <p className="text-sm text-muted-foreground text-center">
          Manage your notifications, system alerts, and preferences in one place.
        </p>
      </div>

      {/* Styled Inline Dropdown Panel */}
      <div 
        className="w-full max-w-2xl min-h-[500px] rounded-2xl border border-rose-100 bg-white shadow-2xl shadow-rose-950/15 flex flex-col overflow-hidden animate-in fade-in slide-in-from-top-3 duration-300"
      >
        {/* Header */}
        <div className="p-5 bg-gradient-to-b from-rose-50 to-white border-b border-rose-100/60 flex items-center justify-between">
          <div>
        ...
        <div className="flex border-b border-slate-100 bg-slate-50/50 p-1.5">
          {(['all', 'unread'] as const).map(tab => (
            <button
              ...
              className={cn(
                "flex-1 py-2.5 text-xs font-bold rounded-lg capitalize transition-all duration-200",
                activeTab === tab 
        ...
        {/* List / Tabs Content */}
        <div className="flex-1 overflow-y-auto min-h-[350px] no-scrollbar">
          ...
          {(activeTab === 'all' || activeTab === 'unread') && (
            filteredNotifications.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center px-6">
                ...
                <p className="text-base font-semibold text-slate-900">Caught Up!</p>
              ...
              <div className="divide-y divide-slate-100">
                {filteredNotifications.map((item) => (
                  <div 
                    ...
                    className={cn(
                      "p-5 flex gap-4 transition-colors duration-200 hover:bg-slate-50 relative group cursor-pointer focus:outline-none focus:bg-slate-50",
                      !item.isRead && "bg-rose-50/20"
                    )}
                  >
                    <div className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 shadow-sm mt-0.5">
                      {getNotificationIcon(item.type)}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-3">
                        <p className={cn("text-sm font-bold text-slate-800", !item.isRead && "text-slate-900 font-extrabold")}>
                          {item.title}
                        </p>
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {!item.isRead && (
                            <button
                              ...
                              className="opacity-100 group-hover:opacity-100 w-6 h-6 rounded bg-emerald-50 text-emerald-600 flex items-center justify-center hover:bg-emerald-500 hover:text-white transition"
                              title="Mark as read"
                            >
